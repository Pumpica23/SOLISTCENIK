document.addEventListener('DOMContentLoaded', () => {
    ...
    async function persistMenu(authToken) {
        if (!currentLang || !authToken) return;
        await fetch(`/api/menu/${currentLang}`, {
            method: 'PUT',
            headers: {
    ...
    function saveData() {
        if (!currentLang || !isAdmin || !adminAuthToken) return;
        if (saveTimer) clearTimeout(saveTimer);
        const tokenForSave = adminAuthToken;
        saveTimer = setTimeout(async () => {
            try {
                await persistMenu(tokenForSave);
            } catch (error) {
                console.error('Failed to save menu data on server:', error);
            }
        }, 250);
    }
    ...
    adminLoginBtn.addEventListener('click', async () => {
        if (isAdmin) {
            const tokenAtLogout = adminAuthToken;
            if (saveTimer) {
                clearTimeout(saveTimer);
                saveTimer = null;
            }
            try {
                await persistMenu(tokenAtLogout);
            } catch (error) {
                console.error('Failed to save during logout:', error);
                alert('Logout save failed. Please check server connection and try again.');
                return;
            }
            isAdmin = false;
            adminAuthToken = null;
